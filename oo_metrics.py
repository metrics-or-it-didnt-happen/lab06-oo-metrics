...
class OOAnalyzer(ast.NodeVisitor):
    ...
    def _compute_dit(self, node: ast.ClassDef) -> int:
        ...
        Simple heuristic: count explicit base classes (ignoring 'object').
        """
        dit = 0
    ...
    def _compute_cbo(self, node: ast.ClassDef) -> int:
        ...
        ignore_names = {
            "self", "cls", "object", "int", "str", "float", "bool", "list",
            "dict", "set", "tuple", "Exception", "ValueError", "TypeError",
            "print", "len", "range", "super", "None", "True", "False",
            "args", "kwargs", "isinstance", "getattr", "setattr", "hasattr",
        ...
        for child in ast.walk(node):
            if isinstance(child, ast.Name):
                name = child.id
                if name not in ignore_names and name not in own_methods and not name.startswith("__"):
                    coupled.add(name)
...
def print_report(classes: list[ClassMetrics]) -> None:
    ...
    print(f"{'Klasa':<35} {'WMC':>5} {'DIT':>5} {'CBO':>5} {'LCOM':>6} "
          f"{'Metod':>6} {'God?':>5}")
    print("-" * 82)
    ...
    for cls in by_wmc[:30]:
        ...
        print(f"  {short_name:<33} {cls.wmc:>5} {cls.dit:>5} {cls.cbo:>5} {cls.lcom:>6.2f} "
